...
| **Explosive** | Athleticism dimension rating | `normalize(ratingAthleticism, 0, 100)` |
...
| **Reliable** | Consecutive sessions with FG% ≥ 40% | `normalize(streak, 0, 12)` |
...
All concrete services conform to their protocol. `SessionFinalizationCoordinator` holds protocol references only.
...
enum SkillRatingCalculator {
    ...
    static func shootingScore(session: TrainingSession) -> Double?
    static func ballHandlingScore(session: TrainingSession) -> Double?
    static func athleticismScore(session: TrainingSession) -> Double?
    static func consistencyScore(session: TrainingSession,
                                 recentSessions: [TrainingSession]) -> Double?
    static func volumeScore(session: TrainingSession,
                            profile: PlayerProfile) -> Double
    static func overallScore(shooting: Double?, handling: Double?,
                             athleticism: Double?, consistency: Double?,
...
`score` returns 0–1, or `nil` if the session's drill type doesn't affect this badge. Internally routes to per-badge private functions — one function per badge. Each function is small, single-purpose, and directly testable.
...
4. Apply EMA to `EarnedBadge.mmr` (create `EarnedBadge` on first earn)
